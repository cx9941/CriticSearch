...
from llm_agent.user_generation import UserLLMGenerationManager, UserGenerationConfig
...
class RayPPOTrainer(object):
    ...
    def _validate(self, identifier):
        ...
        user_gen_config = UserGenerationConfig(
            max_turns=self.config.trainer.max_turns,
            max_start_length=self.config.data.max_start_length,
            max_prompt_length=self.config.data.max_prompt_length,
            max_response_length=self.config.data.max_response_length,
            ...
            num_gpus=self.config.trainer.n_gpus_per_node,
            no_think_rl=self.config.algorithm.no_think_rl,
            llm_ip=self.config.user.llm_ip,
            user_ip=self.config.user.user_ip,
            start_threshold=self.config.user.start_threshold,
            end_threshold=self.config.user.end_threshold,
            topk = self.config.user.topk
        )
        ...
        user_generation_manager = UserLLMGenerationManager(
            tokenizer=self.tokenizer,
            actor_rollout_wg=self.actor_rollout_wg,
            config=user_gen_config,
            is_validation = True,
        )
    ...
    def fit(self):
        ...
        user_gen_config = UserGenerationConfig(
            ...
            start_threshold=self.config.user.start_threshold,
            end_threshold=self.config.user.end_threshold,
            temperature=self.config.user.temperature,
            topk = self.config.user.topk
        )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in tqdm(self.train_dataloader, total=self.total_training_steps, desc="Training"):
                ...
                with _timer('step', timing_raw):
                    if self.config.trainer.do_user:
                        first_input_ids = gen_batch.batch['input_ids'][:, -gen_config.max_start_length:].clone().long()

                        with _timer('gen', timing_raw):
                            user_generation_manager.timing_raw = timing_raw
                            final_gen_batch_output, trajectory_turns = user_generation_manager.run_llm_loop(
                                gen_batch=gen_batch,
                                user_mode=self.config.user.user_mode,
                                initial_input_ids=first_input_ids,
                                current_step=self.global_steps,
                                total_steps=self.config.trainer.total_training_steps,
                            )
                        ...
                        # === 关键：把 question / golden_answers 补回去并尺寸对齐 ===
                        import numpy as np
                        # 基数：user loop 之前（即本轮最初用于生成的）样本数
                        base_n = len(gen_batch.batch)  # gen_batch 的 batch 维度
                        # 目标：user loop 之后的样本数（可能因为多轮生成/rollout而扩大）
                        out_n  = len(final_gen_batch_output.batch)
                        assert out_n % base_n == 0, \
                            f"Shape mismatch: out_n={out_n} is not a multiple of base_n={base_n}"
                        rep = out_n // base_n
                        # 取出并广播上游非张量字段
                        q_base  = gen_batch.non_tensor_batch['question']        # dtype=object 的 np.array
                        ga_base = gen_batch.non_tensor_batch['golden_answers']  # dtype=object 的 np.array
                        q_rep  = np.repeat(q_base,  rep).astype(object)
                        ga_rep = np.repeat(ga_base, rep).astype(object)
                        # 写回到 final_gen_batch_output 的 non_tensor_batch
                        final_gen_batch_output.non_tensor_batch['question']        = q_rep
                        final_gen_batch_output.non_tensor_batch['golden_answers']  = ga_rep
                        gen_batch = final_gen_batch_output.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'], non_tensor_batch_keys=['question', 'golden_answers'])
                    ...
                    if self.config.trainer.do_search:
                        first_input_ids = gen_batch.batch['input_ids'][:, -gen_config.max_start_length:].clone().long()
